...
export default function AreaCliente() {
  ...
  // ✨ Lê qual aba deve abrir através da URL
  const [searchParams, setSearchParams] = useSearchParams();
  const initialTab = searchParams.get('tab') || 'fidelidade';
    ...
    <div className="flex flex-col text-text-base bg-background min-h-screen pb-24 font-sans">
      <div className="w-full max-w-4xl mx-auto px-5 pt-8 md:pt-12 space-y-8">
        ...
        <div className="flex p-1.5 bg-surface border border-border-line rounded-2xl shadow-xs overflow-x-auto hide-scrollbar">
          <button 
            onClick={() => handleTabChange('fidelidade')} 
...
// OS COMPONENTES ABAIXO SE MANTÊM OS MESMOS QUE VOCÊ TINHA (ClubeFidelidade, FormularioPerfil, ListaAgendamentos)
// (Não precisei alterá-los, a menos que você queira. Você pode colar o resto do seu código AreaCliente aqui embaixo)
  ...
  const carregarFidelidade = async () => {
    ...
    try {
      ...
      if (!currentBarbeariaId) {
        const { data: ultimoAgendamento } = await supabase.from('agendamentos').select('barbearia_id').eq('cliente_id', user.id).order('criado_em', { ascending: false }).limit(1).maybeSingle();
        if (ultimoAgendamento) currentBarbeariaId = ultimoAgendamento.barbearia_id;
      }
      if (currentBarbeariaId) {
        const { data: recData } = await supabase.from('recompensas_fidelidade').select('*').eq('barbearia_id', currentBarbeariaId).eq('ativo', true).order('pontos_necessarios', { ascending: true });
        if (recData) setRecompensas(recData);
      }
      const { data: vouchData } = await supabase.from('resgates_fidelidade').select('*').eq('cliente_id', user.id).eq('status', 'pendente').order('criado_em', { ascending: false });
  ...
  const resgatarPremio = async (recompensa) => {
    ...
    try {
      ...
      const { data: sucesso, error } = await supabase.rpc('gerar_codigo_resgate', {
        p_cliente_id: user.id, p_barbearia_id: recompensa.barbearia_id, p_recompensa_id: recompensa.id,
        p_recompensa_nome: recompensa.titulo, p_custo: recompensa.pontos_necessarios, p_codigo: codigoGerado
      });
    ...
    } catch (err) { alert('Erro ao resgatar o prêmio: ' + err.message); } finally { setResgatando(false); }
  };
    ...
    <div className="space-y-8 animate-fadeIn">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {vouchers.map(v => (
              <div key={v.id} className="bg-brand text-white p-6 rounded-3xl relative overflow-hidden shadow-lg shadow-brand/30">
                <div className="absolute -right-4 -bottom-4 bg-white/10 p-8 rounded-full"><Gift size={64} className="text-white/20" /></div>
      ...
      <div className="space-y-4">
        ...
        {recompensas.length === 0 ? (
          <div className="bg-surface p-8 border border-dashed border-border-line rounded-3xl text-center"><p className="font-bold text-text-muted">Nenhum prêmio disponível na barbearia visitada.</p></div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {recompensas.map(rec => {
                ...
                <div key={rec.id} className="bg-surface p-5 sm:p-6 rounded-3xl border border-border-line shadow-sm flex flex-col justify-between group hover:border-brand/40 transition-colors">
                  <div>
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <span className="shrink-0 bg-brand/10 text-brand px-2.5 py-1 rounded-lg text-xs font-black flex items-center gap-1"><Coins size={12}/> {rec.pontos_necessarios}</span>
                    </div>
                  ...
                  <div>
                    <div className="mb-4">
                      <div className="flex justify-between text-[10px] font-black uppercase text-text-muted mb-1.5">
                        <span>Progresso</span><span>{percentual >= 100 ? 'Liberado!' : `Faltam ${falta} moedas`}</span>
                      </div>
                      <div className="w-full h-2 bg-background border border-border-line rounded-full overflow-hidden">
                        <div className={`h-full transition-all duration-1000 ease-out ${podeResgatar ? 'bg-green-500' : 'bg-brand'}`} style={{ width: `${percentual}%` }}></div>
                      </div>
                    </div>
                    <button onClick={() => resgatarPremio(rec)} disabled={!podeResgatar || resgatando} className={`w-full py-3.5 rounded-2xl text-sm font-black transition-all flex items-center justify-center gap-2 cursor-pointer ${podeResgatar ? 'bg-brand text-white shadow-md hover:brightness-105' : 'bg-background border border-border-line text-text-muted opacity-60 cursor-not-allowed'}`}>
...
function FormularioPerfil({ user, profile }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      setNovaSenha('');
    } catch (err) { alert('Erro ao atualizar: ' + err.message); } finally { setLoading(false); }
  };
    ...
    <div className="bg-surface p-6 sm:p-8 rounded-[2rem] border border-border-line shadow-sm space-y-8 animate-fadeIn">
      ...
      <div className="pt-8 border-t border-border-line">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">Nome Completo</label><input type="text" className="w-full bg-background border border-border-line text-sm font-bold text-text-base rounded-2xl px-4 py-3.5 outline-none focus:border-brand" value={formData.nome || ''} onChange={e => setFormData({...formData, nome: e.target.value})} /></div>
          <div><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">E-mail (Login)</label><input type="email" disabled className="w-full bg-background/50 border border-border-line text-sm font-bold text-text-muted rounded-2xl px-4 py-3.5 outline-none cursor-not-allowed" value={user?.email || ''} /></div>
          <div><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">WhatsApp</label><div className="relative"><Phone size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted" /><input type="tel" className="w-full bg-background border border-border-line text-sm font-bold text-text-base rounded-2xl py-3.5 pl-12 pr-4 outline-none focus:border-brand" value={formData.whatsapp || ''} onChange={handlePhoneChange} /></div></div>
          <div><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">CPF</label><input type="text" className="w-full bg-background border border-border-line text-sm font-bold text-text-base rounded-2xl px-4 py-3.5 outline-none focus:border-brand" value={formData.cpf || ''} onChange={handleCpfChange} /></div>
          <div className="md:col-span-2"><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">Endereço Completo</label><div className="relative"><MapPin size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted" /><input type="text" className="w-full bg-background border border-border-line text-sm font-bold text-text-base rounded-2xl py-3.5 pl-12 pr-4 outline-none focus:border-brand" value={formData.endereco || ''} onChange={e => setFormData({...formData, endereco: e.target.value})} /></div></div>
        </div>
      </div>
      <div className="pt-8 border-t border-border-line">
        ...
        <div className="max-w-md"><label className="block text-[10px] font-black text-text-muted uppercase tracking-wider mb-2">Alterar Senha de Acesso</label><input type="password" className="w-full bg-background border border-border-line text-sm font-bold text-text-base rounded-2xl px-4 py-3.5 outline-none focus:border-brand" placeholder="Digite a nova senha (opcional)" value={novaSenha} onChange={e => setNovaSenha(e.target.value)} /></div>
      </div>
      <div className="pt-4">
            ...
            <div key={ag.id} className={`bg-surface p-6 rounded-[2rem] border shadow-sm transition-all ${isCancelado ? 'border-red-500/20 opacity-75 grayscale' : isConcluido ? 'border-green-500/20' : isAusente ? 'border-amber-500/20 opacity-75' : 'border-border-line'}`}>
              ...
              <div className="flex flex-col sm:flex-row sm:items-center gap-4 sm:gap-8">
                <div className="flex items-center gap-3">
                  ...
                  <div><p className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Serviço</p><p className="font-black text-text-base text-sm">{ag.servicos?.nome_servico} <span className="font-normal text-text-muted">({ag.servicos?.duracao_minutos} min)</span></p></div>
                </div>
                <div className="flex items-center gap-3">
                  <div className="bg-background border border-border-line p-2.5 rounded-full text-text-muted"><User size={18}/></div>
                  <div><p className="text-[10px] font-bold text-text-muted uppercase tracking-wider">Profissional</p><p className="font-bold text-text-base text-sm">{ag.barbeiros?.nome}</p></div>
                </div>
              </div>
            </div>
          );
